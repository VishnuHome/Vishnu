    ...
    public class DynamicUserControlBase : UserControl, IDisposable
    {
        ...
        protected virtual void OnDynamicUserControl_ContentRendered()
        {
            if (!System.ComponentModel.DesignerProperties.GetIsInDesignMode(new DependencyObject()))
            {
                ...
                {
                    ((IVishnuViewModel)dataContext).ParentView = this;
                }
                else
                {
                ...
                {
                    ((IVishnuRenderWatcher)dataContext).UserControlContentRendered(this);
                }
            }
            try
            {
        ...
        private void DynamicUserControlBase_IsVisibleChanged(object sender, DependencyPropertyChangedEventArgs e)
        {
            ...
            {
                Dispatcher.BeginInvoke(new Action<ContentControl>(this.waitForContentRendered),
                    System.Windows.Threading.DispatcherPriority.Background /* geht nicht: Normal, ApplicationIdle */, new object[] { (ContentControl)sender });
            }
        }
