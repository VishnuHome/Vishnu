    ...
    public class SingleNodeViewModel : LogicalNodeViewModel
    {
        ...
        public override string ToString()
        {
            ...
            if (logicalNode != null)
            {
                ...
                {
                    NodeCheckerBase? checker = ((SingleNode)logicalNode).Checker;
                    if (checker != null)
                    {
                        if (checker is CheckerShell)
                        {
                            slavePathName = ((CheckerShell)checker).SlavePathName;
                            checkerParameters = ((CheckerShell)checker).OriginalCheckerParameters?.ToString();
                            referencedNodeName = ((CheckerShell)checker).ReferencedNodeName;
                        }
                        else
                        {
                            slavePathName = ((ValueModifier<object>)checker).SlavePathName;
                            referencedNodeName = ((ValueModifier<object>)checker).ReferencedNodeName;
                        }
                    }
                }
                else
                {
                    ...
                    {
                        if (((NodeConnector)logicalNode).Checker != null)
                        {
                            NodeCheckerBase nodeCheckerBase = ((NodeConnector)logicalNode).Checker
                                ?? throw new ArgumentException("NodeConnector ohne Checker und ohne ValueModifier.");
                            if (nodeCheckerBase is CheckerShell)
                            {
                                slavePathName = ((CheckerShell)nodeCheckerBase).SlavePathName;
                                checkerParameters = ((CheckerShell)nodeCheckerBase).OriginalCheckerParameters?.ToString();
                                referencedNodeName = ((CheckerShell)nodeCheckerBase).ReferencedNodeName;
                            }
                            else
                            {
                                slavePathName = ((ValueModifier<object>)nodeCheckerBase).SlavePathName;
                                referencedNodeName = ((ValueModifier<object>)nodeCheckerBase).ReferencedNodeName;
                            }
                        }
                    }
                }
        ...
        public override bool Equals(object? obj)
        {
            ...
            return this.ToString() == ((SingleNodeViewModel)obj).ToString();
        }
        ...
        private void runTaskTreeExecute(object? parameter)
        {
            ...
            {
                ((NodeConnector)this._myLogicalNode).UserRun();
            }
        }
        ...
        private void runOrBreakTaskTreeExecute(object? parameter)
        {
            ...
            else
            {
                ...
                {
                    ((NodeConnector)this._myLogicalNode).UserBreak();
                }
            }
        }
